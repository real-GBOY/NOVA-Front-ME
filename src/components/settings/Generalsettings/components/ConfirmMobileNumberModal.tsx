...
type ConfirmMobileNumberModalProps = {
	isOpen: boolean;
	onClose: () => void;
...
function ConfirmMobileNumberModal({
	isOpen,
	onClose,
	onConfirm,
	onResend,
	email,
}: ConfirmMobileNumberModalProps) {
	...
	const isDirty = useMemo(() => code.some((digit) => digit !== ""), [code]);
	...
	return (
		<>
			<Modal
				...
				title={t("generalSettingsModals.confirmMobile.title")}
				width="max-w-md"
				footer={
					<div className="flex items-center justify-end gap-3">
						<Button variant="secondary" onClick={handleRequestClose}>
							{t("generalSettingsModals.confirmMobile.cancel")}
						</Button>
						<Button
							onClick={handleConfirm}
							disabled={!isCodeComplete}
							className="disabled:opacity-50 disabled:cursor-not-allowed">
							{t("generalSettingsModals.confirmMobile.confirm")}
						</Button>
					</div>
				}>
				<div className="flex flex-col gap-4">
					<p className="text-sm text-text-sub">
						{t("generalSettingsModals.confirmMobile.description")}{" "}
					...
					{canResend ? (
						<>
							<span className="text-text-sub">
								{t("generalSettingsModals.confirmMobile.resendPrompt")}
							</span>
							<Button
								variant="secondary"
								onClick={handleResend}
								className="px-0 py-0 h-auto bg-transparent border-0 text-primary font-medium hover:underline shadow-none">
								{t("generalSettingsModals.confirmMobile.resend")}
							</Button>
						</>
					) : (
						<span className="text-text-sub">
							{t("generalSettingsModals.confirmMobile.resendAfter", {
