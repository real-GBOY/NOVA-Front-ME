...
interface CustomersFloatingActionBarProps {
...
const getCustomerInvoiceCount = (customer?: Customer | null) => {
...
function CustomersFloatingActionBar({
	selectedCount,
	selectedRows,
	onDelete,
	onEdit,
	...
	canEdit = true,
	canDelete = true,
}: CustomersFloatingActionBarProps) {
	...
	const safeRows = displayRows.filter(
		(customer): customer is Customer =>
			Boolean(customer && (customer.customer_id ?? customer.id))
	);
	const displayCount =
	...
	const rowsWithDependencies = safeRows.filter(
		(customer) => getCustomerInvoiceCount(customer) > 0
	);
	...
	const singleActions: ActionButton[] = [
		...(canDelete
			? [
					{
						id: "delete",
						label: t("invoiceProfiles.deleteCustomer"),
		...
		...(canEdit
			? [
					{
						id: "edit",
						label: t("invoiceProfiles.editCustomer"),
						icon: <Edit size={20} />,
						onClick: () => {
	...
	const multiActions: ActionButton[] = [
		...(canDelete
			? [
					{
						id: "delete",
						label: t("invoiceProfiles.deleteCustomers"),
						icon: <Trash size={20} className='fill-danger' />,
						onClick: () => onDelete(safeRows),
						variant: "danger" as const,
						disabled: hasDependencies,
					...
					<div
						...
						<div className='flex items-center gap-2 px-3 py-2'>
							<p className='text-sm text-text-strong whitespace-nowrap'>
								({displayCount}) {t("invoiceProfiles.customersSelected")}
							</p>
						</div>
...
export default CustomersFloatingActionBar;
