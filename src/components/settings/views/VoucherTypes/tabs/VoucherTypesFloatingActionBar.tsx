...
interface VoucherTypesFloatingActionBarProps {
   selectedCount: number;
   selectedRows: VoucherType[];
   onDelete: (voucherTypes: VoucherType[]) => void;
...
function VoucherTypesFloatingActionBar({
   selectedCount,
   selectedRows,
   onDelete,
   onEdit,
   resetSignal,
}: VoucherTypesFloatingActionBarProps) {
   ...
   const singleActions: ActionButton[] = [
      {
         id: "delete",
         label: t("voucherTypes.floatingBar.delete"),
      ...
      {
         id: "edit",
         label: t("voucherTypes.floatingBar.edit"),
         icon: <Edit size={20} />,
   ...
   const multiActions: ActionButton[] = [
      {
         id: "delete",
         label: t("voucherTypes.floatingBar.deleteMultiple"),
         icon: <Trash size={20} className="fill-danger" />,
         onClick: () => onDelete(displayRows),
         variant: "danger",
      },
               ...
               <div
                  ...
                  <div className="flex items-center gap-2 px-3 py-2">
                     <p className="text-sm text-text-strong whitespace-nowrap">
                        ({displayCount}){" "}
                        {t("voucherTypes.floatingBar.selected")}
                     </p>
                  </div>
...
export default VoucherTypesFloatingActionBar;
