...
interface PrettyCashNamesFloatingActionBarProps {
   selectedCount: number;
   selectedRows: PrettyCashName[];
   onDelete: (items: PrettyCashName[]) => void;
   onEdit: (item: PrettyCashName) => void;
   resetSignal?: unknown;
}
...
function PrettyCashNamesFloatingActionBar({
   selectedCount,
   selectedRows,
   onDelete,
   onEdit,
   resetSignal,
}: PrettyCashNamesFloatingActionBarProps) {
   ...
   const singleActions: ActionButton[] = [
      {
         id: "delete",
         label: t("prettyCashNames.floatingBar.delete"),
      ...
      {
         id: "edit",
         label: t("prettyCashNames.floatingBar.edit"),
         icon: <Edit size={20} />,
   ...
   const multiActions: ActionButton[] = [
      {
         id: "delete",
         label: t("prettyCashNames.floatingBar.deleteMultiple"),
         icon: <Trash size={20} className="fill-danger" />,
         onClick: () => onDelete(displayRows),
         variant: "danger",
      },
               ...
               <div
                  ...
                  <div className="flex items-center gap-2 px-3 py-2">
                     <p className="text-sm text-text-strong whitespace-nowrap">
                        ({displayCount}){" "}
                        {t("prettyCashNames.floatingBar.selected")}
                     </p>
                  </div>
...
export default PrettyCashNamesFloatingActionBar;
