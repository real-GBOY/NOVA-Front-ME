...
interface CategoriesFloatingActionBarProps {
   selectedCount: number;
   selectedRows: Category[];
   onDelete: (categories: Category[]) => void;
...
function CategoriesFloatingActionBar({
   selectedCount,
   selectedRows,
   onDelete,
   onEdit,
   ...
   canEdit = true,
   canDelete = true,
}: CategoriesFloatingActionBarProps) {
   ...
   const safeRows = displayRows.filter(
      (cat): cat is Category => Boolean(cat && cat.id !== undefined)
   );
   const displayCount =
   ...
   const singleActions: ActionButton[] = [
      ...(canDelete
         ? [
              {
                 id: "delete",
                 label: t("serviceCatalog.deleteCategory"),
      ...
      ...(canEdit
         ? [
              {
                 id: "edit",
                 label: t("serviceCatalog.editCategory"),
                 icon: <Edit size={20} />,
                 onClick: () => {
   ...
   const multiActions: ActionButton[] = [
      ...(canDelete
         ? [
              {
                 id: "delete",
                 label: t("serviceCatalog.deleteCategories"),
                 icon: <Trash size={20} className="fill-danger" />,
                 onClick: () => onDelete(safeRows),
                 variant: "danger" as const,
                 disabled: hasDependencies,
               ...
               <div
                  ...
                  <div className="flex items-center gap-2 px-3 py-2">
                     <p className="text-sm text-text-strong whitespace-nowrap">
                        ({displayCount}){" "}
                        {t("serviceCatalog.categoriesSelected")}
                     </p>
                  </div>
...
export default CategoriesFloatingActionBar;
