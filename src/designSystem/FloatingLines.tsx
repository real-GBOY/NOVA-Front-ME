...
const vec3 BLACK = vec3(8.0, 8.0, 10.0) / 255.0;
const vec3 RED   = vec3(193.0, 18.0, 31.0) / 255.0;
const vec3 GREEN = vec3(0.0, 122.0, 61.0) / 255.0;
const vec3 WHITE = vec3(235.0, 235.0, 235.0) / 255.0;
...
vec3 background_color(vec2 uv) {
  ...
  // Horizontal flag-inspired bands (muted for low brightness)
  float stripeHeight = 0.66;
  float topBand = smoothstep(0.2, stripeHeight, uv.y);
  float midBand = smoothstep(-0.15, 0.15, uv.y);
  float bottomBand = smoothstep(-stripeHeight, -0.2, uv.y);

  vec3 greenBand = mix(BLACK, GREEN, 0.55);
  vec3 whiteBand = mix(BLACK, WHITE, 0.35);
  vec3 blackBand = mix(BLACK, BLACK, 0.9);

  col = mix(col, greenBand, topBand);
  col = mix(col, whiteBand, midBand);
  col = mix(col, blackBand, bottomBand);

  // Vertical muted red stripe on the left to echo the flag
  float redStripe = smoothstep(-0.25, 0.35, uv.x);
  col = mix(col, RED * 0.55 + BLACK * 0.45, 1.0 - redStripe);

  // Subtle vignette to keep focus near center
  float vignette = smoothstep(0.0, 1.2, length(uv) * 0.8);
  col *= mix(0.95, 0.7, vignette);
...
vec3 getLineColor(float t, vec3 baseColor) {
  ...
  return mix(gradientColor, baseColor, 0.35);
}
...
export default function FloatingLines({
   linesGradient = ["#007A3D", "#271e1e", "#C1121F", "#0e0707"],
